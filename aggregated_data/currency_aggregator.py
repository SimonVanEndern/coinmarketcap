...
from aggregated_data import financial_data_calculator
from aggregated_data.financial_data_calculator import FinancialDataCalculator
...
class CurrencyAggregator(DTO):
    def __init__(self, currency: str, last_time: int, interval: int):
        ...
        self.compressed_only_raw_data_folder: str = os.path.join(LocalData.EXTERNAL_PATH_COMPRESSED_DATA,
                                                                 LocalData.FOLDER_COMPRESSED_DATA_ONLY_RAW_DATA,
                                                                 self.currency)

        self.compressed_with_additional_data_folder: str = os.path.join(LocalData.EXTERNAL_PATH_COMPRESSED_DATA,
                                                                        LocalData.FOLDER_COMPRESSED_DATA_WITH_ADDITIONAL_DATA,
                                                                        self.currency)

        self.aggregated_with_additional_data_folder: str = os.path.join(LocalData.EXTERNAL_PATH_AGGREGATED_DATA,
                                                                        LocalData.FOLDER_COMPRESSED_DATA_WITH_ADDITIONAL_DATA,
                                                                        self.currency)
